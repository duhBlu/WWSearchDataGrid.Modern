...
{
    /// <summary>
    /// Backs the Edit Entire Row sample — an employee roster edited a whole row at a time. The grid's
    /// <see cref="SearchDataGrid.RowEditTrigger"/> (driven by <see cref="SelectedTrigger"/>) decides
    /// when a row promotes into full-row edit mode; <see cref="EditRowItem"/> implements
    /// <see cref="System.ComponentModel.IEditableObject"/> so Cancel reverts the whole row.
    /// </summary>
    public sealed partial class EditEntireRowSampleViewModel : SampleViewModelBase
    {
        [ObservableProperty]
        ...
        /// <summary>
        /// Starts on <see cref="RowEditTrigger.OnCellEditorOpen"/> so the full-row overlay is visible
        /// the moment the user clicks into any cell — the headline behaviour of the sample.
        /// </summary>
        [ObservableProperty]
        ...
        public EditEntireRowSampleViewModel()
        {
            StartLoad(() =>
            {
