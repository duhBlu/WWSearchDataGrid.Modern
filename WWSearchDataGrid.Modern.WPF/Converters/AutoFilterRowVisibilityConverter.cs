...
namespace WWSearchDataGrid.Modern.WPF.Converters
{
    ...
    /// <see cref="SearchDataGrid.ShowAutoFilterRow"/> is <c>true</c> <em>and</em>
    /// <see cref="SearchDataGrid.AutoFilterRowPosition"/> is
    /// <see cref="WPF.AutoFilterRowPosition.Cell"/>; <see cref="Visibility.Collapsed"/>
    /// otherwise. Drives the dedicated pinned filter row in the
    /// <see cref="SearchDataGrid"/> template.
    /// </summary>
    /// <remarks>
    /// Header-mode placement reuses the column header chrome rather than the pinned row,
    /// so the row collapses entirely whenever the position is not <c>Cell</c> — collapsing
    /// (rather than hiding) saves the measure + arrange pass for every column's filter
    /// editor when the row isn't in use.
    /// </remarks>
    public sealed class AutoFilterRowVisibilityConverter : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            bool isCellPosition = values[1] is AutoFilterRowPosition pos && pos == AutoFilterRowPosition.Cell;
            bool pinnedRowActive = show && isCellPosition;
