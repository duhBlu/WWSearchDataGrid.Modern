...
namespace WWSearchDataGrid.Modern.WPF
{
    public partial class SearchDataGrid
    {
        ...
        /// FilterPanel chip strip stay populated, but when this tree is set, row-level
        ...
        private async void FilterItemsSourceCore(int delay)
        {
            try
            {
                ...
                if (FilterPanel?.FiltersEnabled == true)
                {
                    ...
                    // per-column slices so the per-column popup and FilterPanel chip strip stay
        ...
        /// <summary>
        /// Projects <see cref="GridFilterTree"/> into the FilterPanel chip model so the chip
        ...
        private IEnumerable<ColumnFilterInfo> GetActiveColumnFiltersFromTree(FilterGroupNode root)
        {
            ...
            // meaning (each bracket would AND to its neighbor).
            if (root.Operator != LogicalOperator.And)
            {
                var bracket = BuildUnifiedBracket(root, new FilterEditorNode[] { root }, isLeadingBracket: true);
                return bracket == null
        ...
        private void OnOperatorToggled(object sender, OperatorToggledEventArgs e)
        {
            try
            {
                ...
                // the simple flat-tree case the FilterPanel toggle can produce).
                ...
                if (e.OperatorToken?.SourceFilter?.FilterData is IColumnFilterHost columnSearchBox)
                {
                    ...
                    if (e.Level == OperatorLevel.Group)
                    {
                        ...
                        // tree so the editor and FilterPanel render cross-column groupings as a
