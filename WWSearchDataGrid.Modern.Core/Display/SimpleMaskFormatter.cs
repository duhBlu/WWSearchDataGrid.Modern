...
{
    /// <summary>
    /// Region-based mask engine for the <see cref="MaskType.Simple"/> grammar — literal characters
    /// interleaved with required / optional placeholder slots, plus <c>+</c> / <c>*</c> quantifiers
    /// for variable-length regions. Date / TimeSpan engines (added in later phases) translate
    /// their format strings into this same region model and delegate here.
    ///
    /// Mask syntax:
    ...
    public class SimpleMaskFormatter : IMaskFormatter
    {
        ...
        /// <summary>
        /// Creates a new <see cref="SimpleMaskFormatter"/> with the specified mask pattern.
        /// </summary>
        ...
        public SimpleMaskFormatter(string mask, char promptChar = '_')
        {
        ...
        /// <summary>
        /// Finalizes the mask state for focus loss. Returns the current display text without
        /// mutating regions — empty slots stay as prompt characters and partial input is
        /// preserved as-is.
        /// </summary>
        /// <remarks>
        /// Earlier versions filled empty required slots with type defaults ('0' for digits,
        /// 'a' for letters), but that's hostile UX: tabbing through an empty <c>(000) 000-0000</c>
        /// cell turned <c>(___) ___-____</c> into <c>(000) 000-0000</c> and committed that to
        /// the bound source. Now Finalize is non-mutating; <see cref="UnmaskedValue"/> reflects
        /// what the user actually entered, and <c>MaskInputBehavior.OnLostFocus</c> clears the
        /// textbox entirely when no input was given.
        /// </remarks>
