...
// Themes/Generic.xaml. Without it, controls whose DefaultStyleKey type lives here (e.g. WWTextBox,
// which keys off WWEditorBase) never resolve a template, because theme-style lookup consults only the
...
//
// This assembly holds the editor *controls* (ns WWControls.Wpf.Editors — WWEditorBase, the WWxxxEdit
// family, SegmentedDateTimeEditor, EditorThemeKeys) AND the grid-agnostic EditSettings
// *adapters* (ns WWControls.Wpf.Editors.Settings — BaseEditorSettings, TextBoxSettings, ...). Both
// namespaces map to the one `ww` xmlns so consumer XAML reaches `ww:WWTextBox` and
// `ww:TextBoxSettings` alike. The grid connects to the adapters purely via the column EditSettings
