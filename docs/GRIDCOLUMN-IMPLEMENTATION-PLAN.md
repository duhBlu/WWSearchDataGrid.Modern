...
**Acceptance criteria:**
- [x] `FieldType` auto-populated for all columns when ItemsSource is set
- [x] Explicit `FieldType` on GridColumn overrides auto-detection
- [x] Nullable types resolved correctly (e.g., `decimal?` → Number)
...
**Acceptance criteria:**
- [x] Bool columns auto-generate checkbox search without `UseCheckBoxInSearchBox="True"`
- [x] User-set properties always win over auto-configuration
- [x] SampleApp columns can be simplified (remove redundant property settings)

**Deferred from 2.2:** Populating the search dropdown for true `enum` types from `Enum.GetValues`. The current dropdown is data-driven via `SetupColumnDataLazy` and would need a new "static value source" path on `SearchTemplateController`. Marked with a `TODO` in `GridColumn.ApplyTypeBasedDefaults`.

### 2.3 Auto-Generated Columns + DataTable Support

**Description:** Two changes that ship together because they share a code path:

1. **Type resolver / property enumerator overhaul.** Switch from raw reflection to `TypeDescriptor.GetProperties(firstItem)` as the fallback after `IItemProperties`. This single change makes `DataTable` / `DataView` / `DataRowView` work for free because `DataView` exposes column metadata via `ITypedList` → `PropertyDescriptor`. Same code path still handles POCOs and anonymous types.
2. **Auto-generation.** When `AutoGenerateColumns="True"` and `GridColumns` is empty, walk the resolved property descriptors and auto-create `GridColumn` instances.

**In scope:**
- `IEnumerable<T>` of POCOs / ViewModels / anonymous types (already works — verify no regression)
- `DataTable`, `DataTable.DefaultView`, `DataView` — primary new capability

**Out of scope (deferred until a real case appears):**
- XML / `XmlDataProvider` / `XElement` — needs XPath traversal, fundamentally different binding model
- `IDictionary<string,object>` / `ExpandoObject` — string-keyed, no descriptors
- Custom `ICustomTypeDescriptor` / `ITypedList` beyond what the framework already plumbs through

**DataTable-specific edge cases to handle:**
- Empty table → fall back from "first item" to `IItemProperties` (always present on DataView, even with zero rows)
- `DBNull.Value` → existing IsNull/IsNotNull evaluators must treat it like `null`. Verify and patch only if broken.
- Column names with spaces / special chars → confirm `new Binding(columnName)` resolves at runtime (it should, via PropertyDescriptor lookup, no `[brackets]` needed)
- Computed/expression columns → expose a CLR type via the column descriptor; should "just work"

**Acceptance criteria:**
- [ ] Type resolver uses `IItemProperties` → `TypeDescriptor.GetProperties(firstItem)` → reflection fallback chain
- [ ] DataTable test page in SampleApp (vendor-product-editor-shaped grid) renders with correct types per column
- [ ] DBNull values filter correctly with IsNull / IsNotNull
...
- [ ] Existing POCO sample app exhibits no regressions
